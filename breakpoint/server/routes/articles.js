...
// Artikel erstellen
router.post("/", (req, res) => {
  const { title, author_id, category, image_url } = req.body;
  ...
  const sql =
    "INSERT INTO articles (title, author_id, category, image_url, views) VALUES (?, ?, ?, ?, 0)";
  db.query(sql, [title, author_id, category, image_url], (err, result) => {
